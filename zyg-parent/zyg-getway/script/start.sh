...
docker push "$docker_repostory$image_name"
	...
	echo "images run"
	ssh root@$ip_address docker run --name $server_name"-`date +%m%d`" --restart=always -d -v /data/zyg/logs/zyg-getway/"`date +%m%d`":/log  --network=host "$docker_repostory$image_name"
	echo "docker images run complete:$image_name"
	exit 1
fi