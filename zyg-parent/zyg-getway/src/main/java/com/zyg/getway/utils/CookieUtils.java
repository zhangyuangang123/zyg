...
public final class CookieUtils {
	...
	public static String getCookieValue(ServerHttpRequest request , String cookieName, boolean isDecoder) {
		...
		for (Map.Entry<String, List<HttpCookie>> entry : cookies.entrySet()) {
			System.out.println("cookies key = " + entry.getKey() + ", value = " + entry.getValue());
			if(cookieName.equals(entry.getKey())) {
				List<HttpCookie> httpCookies = entry.getValue();
				for (HttpCookie httpCookie : httpCookies){
					System.out.println("httpCookies key = " + httpCookie.getName() + ", value = " + httpCookie.getValue());
					if (httpCookie.getName().equals(cookieName)) {
						retValue = httpCookie.getValue();
						break;
					}
				}
				break;
